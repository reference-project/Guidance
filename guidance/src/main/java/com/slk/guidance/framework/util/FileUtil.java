package com.slk.guidance.framework.util;
...
//    private static String serverbucketName="";
	 ...
	 */
	public static String FileByteImgUp(String imgurl,String picPath){
		// 创建OSSClient实例
		OSSClient ossClient = new OSSClient(endpoint, accessKeyId, accessKeySecret);
	...
	/**
	 * 文件流上传文件
	 * Create by song on 2018-04-02 10:03:02
	 */
	public static String setFileInputStream(String url ,InputStream inputStream){
		// 获取当前年份、月份
		String formatDate = new SimpleDateFormat("yyyy-MM").format(new Date());
		// 创建OSSClient实例
		OSSClient ossClient = new OSSClient(endpoint, accessKeyId, accessKeySecret);
		//阿里云存储地址
		String qrcodeUrl = "WphGuidance/"+url+"/"+formatDate+"/qrcode_"+RandomFileName()+".jpg";
		// 上传文件流
		ossClient.putObject(appbucketName, qrcodeUrl, inputStream);
		// 关闭client
		ossClient.shutdown();

		return "http://slk-save.oss-cn-beijing.aliyuncs.com/"+qrcodeUrl;
	}
	 ...
	 * @throws IOException
	 */
	public static String uploadsFile(String picPath,MultipartFile[] mf)  {
		String filePath="";
		// 获取当前年份、月份
		String formatDate = new SimpleDateFormat("yyyy-MM").format(new Date());
		...
		String path = projectPath +"/"+projectName+"/"+picPath+"/"+formatDate+"/"+ fileName;

		
		File newFile = new File(projectPath +"/"+projectName+"/"+picPath+"/");
		if (!newFile.exists() && !newFile.isDirectory()) {
			newFile.mkdirs();
		}
		File localFile = new File(path);
			...
			filePath=projectName+"/"+picPath+"/"+ fileName;
		}
